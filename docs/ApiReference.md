...
| `setOnGlobalInit(OnGlobalInitCallback)` | callback: 全局初始化回调 | void | 设置全局初始化配置回调，详见 [自定义配置](./advanced/CustomConfig.md) |
...
| 方法 | 参数 | 返回值 | 说明 |
|-----|------|-------|------|
| `attach(AliPlayerController)` | controller | void | 绑定控制器（自动使用默认插槽） |
| `detach()` | - | void | 解绑 UI，仅解除视图与控制器的关联 |
...
| `getSlotManager()` | - | SlotManager | 获取插槽管理器 |
...
| 方法 | 参数 | 返回值 | 说明 |
|-----|------|-------|------|
| `configure(AliPlayerModel)` | model: 播放数据 | void | 配置播放数据，**必须在 `attach()` 之前调用** |
| `getModel()` | - | AliPlayerModel | 获取当前播放数据配置 |
| `getPlayer()` | - | IMediaPlayer | 获取播放器实例 |
| `destroy()` | - | void | 销毁播放器并释放所有资源，应在 `onDestroy()` 中调用 |
...
| `onPlayerConfig(OnPlayerConfigCallback)` | 实例配置回调 | 设置实例级播放器配置回调（可选），详见 [自定义配置](./advanced/CustomConfig.md) |
...
### **2.1 SlotManager（插槽管理器）**

`SlotManager` 是插槽系统的统一入口，提供插槽的注册、可见性控制和管理能力，通过 `AliPlayerView.getSlotManager()` 获取。
...
| 方法 | 参数 | 说明 |
|-----|------|------|
| `hide(SlotType)` | type: 插槽类型 | 隐藏整个插槽 |
| `show(SlotType)` | type: 插槽类型 | 显示之前隐藏的插槽 |
| `hideElements(SlotType, String...)` | type, elementKeys | 隐藏插槽内特定元素 |
| `showElements(SlotType, String...)` | type, elementKeys | 显示插槽内之前隐藏的元素 |
| `setHiddenElements(SlotType, Set<String>)` | type, elementKeys | 设置插槽的隐藏元素集合（覆盖式） |
| `setAllHiddenElements(Map)` | map | 批量设置多个插槽的隐藏元素 |
...
AliPlayerKit 提供双层级自定义配置机制，允许开发者在关键时机注入自定义 SDK 配置。

### **11.1 OnGlobalInitCallback（全局初始化配置回调）**

全局初始化配置回调接口，在 `AliPlayerKit.init()` 完成后触发，仅执行一次。

| 方法 | 说明 |
|------|------|
| `void onGlobalInit()` | 全局初始化完成后的配置回调 |

**注册方式**: `AliPlayerKit.setOnGlobalInit(OnGlobalInitCallback callback)`
